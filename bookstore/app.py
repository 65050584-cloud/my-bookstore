...
def admin_notifications():
    ...
    try:
        # 1. ดึงการแจ้งเตือนจากตาราง notifications (คนสมัครใหม่, ออเดอร์ใหม่)
        res = supabase.table("notifications").select("*").order("created_at", desc=True).execute()
        notifications = res.data
        
        # 2. เช็คสต๊อกหนังสือ (ดึงเฉพาะเล่มที่เหลือน้อยกว่า 5 เล่ม)
        res_books = supabase.table("books").select("title, stock_quantity").lt("stock_quantity", 5).execute()
        low_stock_books = res_books.data
        
        # 3. เอาหนังสือที่ใกล้หมด มาสร้างเป็นการแจ้งเตือนแบบ Real-time แทรกไว้บนสุด
        if low_stock_books:
            for book in low_stock_books:
                notifications.insert(0, {
                    "type": "stock",
                    "message": f"⚠️ หนังสือ '{book['title']}' ใกล้หมดสต๊อกแล้ว! (เหลือ {book['stock_quantity']} เล่ม)",
                    "created_at": "แจ้งเตือนระบบ",
                    "is_read": False
                })
